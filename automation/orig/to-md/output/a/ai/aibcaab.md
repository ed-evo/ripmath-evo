Quello al denominatore l'ho già calcolato e vale $$-3$$
Devo calcolare il valore del determinante al numeratore:

$$
\begin{vmatrix}
1 & 1 & 1 & -1 \\
1 & -2 & 0 & 1 \\
0 & 2 & -1 & -1 \\
\textcolor{blue}{-2} & \textcolor{blue}{0} & \textcolor{blue}{-1} & \textcolor{blue}{0}
\end{vmatrix}
$$

Anche qui conviene sviluppare secondo l'ultima riga perché ho solo due termini diversi da zero.
La evidenzio in blu.
Il primo termine della riga ha posto $$a_{4,1}$$ cioè quarta riga e prima colonna quindi è di posto dispari e quindi va cambiato di segno.
Il terzo termine della riga ha posto $$a_{4,3}$$ cioè quarta riga e terza colonna quindi è di posto dispari e quindi va cambiato di segno.
Dove c'è zero non devo sviluppare perché zero per numero uguale zero.

$$
\begin{vmatrix}
1 & 1 & 1 & -1 \\
1 & -2 & 0 & 1 \\
0 & 2 & -1 & -1 \\
\textcolor{blue}{-2} & 0 & \textcolor{blue}{-1} & 0
\end{vmatrix} = -\textcolor{blue}{(-2)} \cdot \begin{vmatrix} 1 & 1 & -1 \\ -2 & 0 & 1 \\ 2 & -1 & -1 \end{vmatrix} - (\textcolor{blue}{-1}) \cdot \begin{vmatrix} 1 & 1 & -1 \\ 1 & -2 & 1 \\ 0 & 2 & -1 \end{vmatrix} =
$$

Devo calcolare i due determinanti dopo l'uguale:

***

Calcolo il primo:

$$
\begin{vmatrix}
1 & 1 & -1 \\
-2 & 0 & 1 \\
2 & -1 & -1
\end{vmatrix}
$$

Applichiamo la regola di Sarrus riportando la prima e la seconda colonna:

$$
\begin{matrix}
1 & 1 & -1 & 1 & 1 \\
-2 & 0 & 1 & -2 & 0 \\
2 & -1 & -1 & 2 & -1
\end{matrix}
$$

Moltiplico prima gli elementi della diagonale principale e delle due diagonali parallele:

$$
\begin{matrix}
\textcolor{fuchsia}{1} & \textcolor{red}{1} & \textcolor{blue}{-1} & 1 & 1 \\
-2 & \textcolor{fuchsia}{0} & \textcolor{red}{1} & \textcolor{blue}{-2} & 0 \\
2 & -1 & \textcolor{fuchsia}{-1} & \textcolor{red}{2} & \textcolor{blue}{-1}
\end{matrix}
$$

$$
= \textcolor{fuchsia}{1 \cdot 0 \cdot (-1)} + \textcolor{red}{1 \cdot 1 \cdot 2} + \textcolor{blue}{(-1) \cdot (-2) \cdot (-1)} = 0 + 2 - 2 = 0
$$

Moltiplico poi gli elementi della diagonale secondaria e delle due diagonali parallele:

$$
\begin{matrix}
1 & 1 & \textcolor{orange}{-1} & \textcolor{green}{1} & \textcolor{purple}{1} \\
-2 & \textcolor{orange}{0} & \textcolor{green}{1} & \textcolor{purple}{-2} & 0 \\
\textcolor{orange}{2} & \textcolor{green}{-1} & \textcolor{purple}{-1} & 2 & -1
\end{matrix}
$$

$$
= \textcolor{orange}{(-1) \cdot 0 \cdot 2} + \textcolor{green}{1 \cdot 1 \cdot (-1)} + \textcolor{purple}{1 \cdot (-2) \cdot (-1)} = 0 - 1 + 2 = 1
$$

Adesso faccio la differenza fra i due valori trovati ed ottengo il valore del primo determinante:

$$
\begin{vmatrix}
1 & 1 & -1 \\
-2 & 0 & 1 \\
2 & -1 & -1
\end{vmatrix} = 0 - 1 = -1
$$

***

Calcolo il secondo determinante:

$$
\begin{vmatrix}
1 & 1 & -1 \\
1 & -2 & 1 \\
0 & 2 & -1
\end{vmatrix}
$$

Applichiamo la regola di Sarrus riportando la prima e la seconda colonna:

$$
\begin{matrix}
1 & 1 & -1 & 1 & -1 \\
1 & -2 & 1 & 1 & -2 \\
0 & 2 & -1 & 0 & 2
\end{matrix}
$$

Moltiplico prima gli elementi della diagonale principale e delle due diagonali parallele:

$$
\begin{matrix}
\textcolor{fuchsia}{1} & \textcolor{red}{1} & \textcolor{blue}{-1} & 1 & 1 \\
1 & \textcolor{fuchsia}{-2} & \textcolor{red}{1} & \textcolor{blue}{1} & -2 \\
0 & 2 & \textcolor{fuchsia}{-1} & \textcolor{red}{0} & \textcolor{blue}{2}
\end{matrix}
$$

$$
= \textcolor{fuchsia}{1 \cdot (-2) \cdot (-1)} + \textcolor{red}{1 \cdot 1 \cdot 0} + \textcolor{blue}{(-1) \cdot 1 \cdot 2} = 2 + 0 - 2 = 0
$$

Moltiplico poi gli elementi della diagonale secondaria e delle due diagonali parallele:

$$
\begin{matrix}
1 & 1 & \textcolor{orange}{-1} & \textcolor{green}{1} & \textcolor{purple}{1} \\
1 & \textcolor{orange}{-2} & \textcolor{green}{1} & \textcolor{purple}{1} & -2 \\
\textcolor{orange}{0} & \textcolor{green}{2} & \textcolor{purple}{-1} & 0 & 2
\end{matrix}
$$

$$
= \textcolor{orange}{(-1) \cdot (-2) \cdot 0} + \textcolor{green}{1 \cdot 1 \cdot 2} + \textcolor{purple}{1 \cdot 1 \cdot (-1)} = 0 + 2 - 1 = 1
$$

Adesso faccio la differenza fra i due valori trovati ed ottengo il valore del secondo determinante:

$$
\begin{vmatrix}
1 & 1 & -1 \\
1 & -2 & 1 \\
0 & 2 & -1
\end{vmatrix} = 0 - 1 = -1
$$

***

Ed ottengo quindi:

$$
\begin{vmatrix}
1 & 1 & 1 & -1 \\
1 & -2 & 0 & 1 \\
0 & 2 & -1 & -1 \\
\textcolor{blue}{-2} & 0 & \textcolor{blue}{-1} & 0
\end{vmatrix} = -\textcolor{blue}{(-2)} \cdot \begin{vmatrix} 1 & 1 & -1 \\ -2 & 0 & 1 \\ 2 & -1 & -1 \end{vmatrix} - (\textcolor{blue}{-1}) \cdot \begin{vmatrix} 1 & 1 & -1 \\ 1 & -2 & 1 \\ 0 & 2 & -1 \end{vmatrix} =
$$

[$$ = -(-2) \cdot (-1) - (-1) \cdot (-1) = -2 - 1 = -3 $$]{.text-red}