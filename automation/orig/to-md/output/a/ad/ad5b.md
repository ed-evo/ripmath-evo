...
Osservando la divisione fra polinomi già vista nella pagina precedente puoi notare che si incolonnano tutti i termini simili (con le stesse lettere alla stessa potenza), quindi non c'è bisogno di scrivere le lettere; basterà scrivere i numeri e le lettere saranno sottointese. Inoltre, poiché divido per un binomio, il risultato avrà un termine meno del polinomio di partenza, quindi l'ultimo termine del polinomio lo scrivo fuori delle sbarre.

> Consideriamo poi che quello che conta nella divisione è il secondo termine del divisore [perché il primo rimoltiplicato viene uguale e di segno contrario, quindi va via]{.text-blue} e siccome quando moltiplico dovrei cambiare di segno, invece di cambiare di segno il risultato cambio lui di segno e lo scrivo a sinistra.

Adesso inizio la divisione: abbasso il primo termine.

Poi lo moltiplico per il numero a sinistra e scrivo il risultato sotto il secondo termine (ma sopra la sbarra orizzontale).
...
Ora su questo argomento è bene fare molti [esercizi](ad5b1.html){.text-red}.