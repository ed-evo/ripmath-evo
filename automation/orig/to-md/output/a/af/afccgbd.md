...
$$
\frac{x^2 - x + 1}{x - 1} - \frac{x^2 + x + 1}{x + 1} = \frac{3x}{x + 1} + \frac{2}{x^2 - 1}
$$
...
È un' [equazione fratta](afbg.html) quindi prima di risolverla dobbiamo porre le condizioni di realtà cioè supporre che i denominatori siano diversi da zero
...
$\textcolor{blue}{\text{m.c.m.} = (x+1)(x-1)}$
...
- Risolvo la seconda:
  ...
  $\textcolor{blue}{x = 1}$
  Però per le condizioni di realtà questo valore non può essere accettato (altrimenti, per togliere i denominatori, avrei moltiplicato per zero entrambi i membri dell'equazione contro il [secondo principio](afbc.html) di equivalenza).

Ho quindi una sola soluzione accettabile

$x_1 = 0$ accettabile $\quad x_2 = 1$ non accettabile