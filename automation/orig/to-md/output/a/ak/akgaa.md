# [esercizio sui radicali in forma esponenziale]{.text-red}

$$
\textcolor{red}{\sqrt[3]{2} \cdot \sqrt[4]{4} \cdot \sqrt{8} =}
$$

trasformo in forma esponenziale

$$
\textcolor{red}{= 2^{1/3} \cdot 4^{1/4} \cdot 8^{1/2} =}
$$

ricordando che $$4 = 2^2$$ ed $$8 = 2^3$$ posso scrivere

$$
\textcolor{red}{= 2^{1/3} \cdot (2^2)^{1/4} \cdot (2^3)^{1/2} =}
$$

eseguo la potenza di potenza

$$
\textcolor{red}{= 2^{1/3} \cdot 2^{2/4} \cdot 2^{3/2} =}
$$

posso semplificare $$2/4 = 1/2$$

$$
\textcolor{red}{= 2^{1/3} \cdot 2^{1/2} \cdot 2^{3/2} =}
$$

prodotto di potenze con la stessa base, posso fare la somma degli esponenti

$$
\textcolor{red}{= 2^{(1/3 + 1/2 + 3/2)} =}
$$

$$
\textcolor{red}{= 2^{(2+3+9)/6} =}
$$

ritrasformo in forma di radice

$$
\textcolor{red}{= \sqrt[6]{2^{14}} =}
$$

estraggo di radice

$$
\textcolor{red}{= 2^2 \sqrt[6]{2^2} =}
$$

semplifico per $$2$$ l'indice di radice con l'esponente di potenza

$$
\textcolor{red}{= 4 \sqrt[3]{2} =}
$$