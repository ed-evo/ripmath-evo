# <span class="text-red-600">Segno di espressioni quoziente di due espressioni elementari</span>
...
Voglio trovare l'insieme dei valori che posso assegnare alla $x$ perché l'espressione sia minore di zero. Essendo questa espressione quoziente di due termini sarà minore di zero quando i due termini che la compongono hanno segno diverso, cioè uno maggiore di zero e l'altro minore oppure viceversa.
...
$$
\begin{cases} x - 2 > 0 \\ x - 4 < 0 \end{cases} \quad \begin{cases} x - 2 < 0 \\ x - 4 > 0 \end{cases}
$$

Capisci che questo sarebbe un metodo molto pesante, soprattutto se invece del prodotto di due termini l'espressione fosse il prodotto di 3, 4, 5... termini. Allora mettiamo in un grafico il segno di ognuno dei termini e poi scegliamo gli intervalli dove i segni sono concordi (entrambi positivi o entrambi negativi).
...
Faccio lo schema. Hai bisogno di [aiuto](agcab0.html) per fare lo schema?

$x > 2 \quad \text{--- (2) ++++++++++++++++++++}$
$x > 4 \quad \text{----------------- (4) +++++++++++}$
<span class="text-blue-500">Espressione $\quad$ ++++ (2) ----- (4) +++++++++++</span>

L'espressione è negativa dove i due fattori sono uno positivo e uno negativo, quindi avremo
...
(Rappresentazione grafica dell'intervallo)

***

Ricapitolando: se devi risolvere una disequazione fratta
...
- <span class="text-blue-500">$\frac{x - 3}{x + 1} > 0$ [Soluzione](agcab1.html)</span>
- <span class="text-blue-500">$\frac{x - 4}{x + 2} \le 0$ [Soluzione](agcab2.html)</span>
- <span class="text-blue-500">$\frac{x^2 - 3x - 10}{x - 1} \ge 0$ [Soluzione](agcab3.html)</span>
- <span class="text-blue-500">$\frac{x^2 - 5x + 6}{x^2 - 4x - 5} \ge 0$ [Soluzione](agcab4.html)</span>