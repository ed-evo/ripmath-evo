...
- Mostriamo che $(H_2, \oplus)$ è un gruppo; devono valere le proprietà:
    ...
    - $\oplus$ è associativa: infatti, chiamati $H_2(A)$, $H_2(B)$ e $H_2(C)$ tre elementi di $H_2$, abbiamo:
      ...
      $$
      Deriva dal fatto che la somma fra numeri naturali è associativa.
...
- Basta mostrare che $\otimes$ è associativa, cioè chiamati $H_2(A)$, $H_2(B)$ e $H_2(C)$ tre elementi di $H_2$, abbiamo sempre:
  $$
  [H_2(A) \cdot H_2(B)] \cdot H_2(C) = H_2(A) \cdot [H_2(B) \cdot H_2(C)]
  $$
  Questo deriva dal fatto che nelle matrici quadrate $2 \times 2$ il prodotto riga per colonna è associativo. Mostriamolo: siccome la dimostrazione è piuttosto lunga, ti faccio un esempio in una pagina a parte: [segui il link](hceafa.html).
