# <span class="text-red-600">Esercizio</span>

Risolvere:
$$
x + 2 + \sqrt{x + 2} = 0
$$
Isolo la radice portando gli altri termini dopo l'uguale:
$$
\sqrt{x + 2} = -x - 2
$$

Pongo le condizioni iniziali: il termine sotto radice deve essere maggiore o uguale a zero e l'altro termine deve avere lo stesso segno del radicale:
$$
\begin{cases} x + 2 \ge 0 \\ -x - 2 \ge 0 \end{cases}
$$
Cambio di segno e di verso la seconda:
$$
\begin{cases} x + 2 \ge 0 \\ x + 2 \le 0 \end{cases}
$$
Risolvo:
$$
\begin{cases} x \ge -2 \\ x \le -2 \end{cases}
$$
Quindi la soluzione è:
$$
x = -2
$$
Quindi saranno accettabili solo le soluzioni che siano uguali a $-2$.

Ora procedo normalmente:
Elevo a quadrato da una parte e dall'altra:
$$
[\sqrt{x + 2}]^2 = (-x - 2)^2
$$
Eseguo il quadrato dopo l'uguale. Prima dell'uguale semplifico il quadrato con la radice:
$$
x + 2 = x^2 + 4x + 4
$$
Trasporto tutto prima dell'uguale:
$$
x + 2 - x^2 - 4x - 4 = 0
$$
Sommo:
$$
-x^2 - 3x - 2 = 0
$$
Cambio segno:
$$
x^2 + 3x + 2 = 0
$$
Risolvo:
$$
x_{1,2} = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}
$$
$$
x_{1,2} = \frac{-3 \pm \sqrt{9 - 8}}{2}
$$
$$
x_{1,2} = \frac{-3 \pm 1}{2}
$$

Le radici sono:
$$
x_1 = \frac{-3 - 1}{2} = -2
$$
$$
x_2 = \frac{-3 + 1}{2} = -1
$$

La prima soluzione $x = -2$ è accettabile, la seconda $x = -1$ non è accettabile.