# <span class="text-red-600">esercizio</span>

Trovare tutte le soluzioni delle equazioni
$$
x^3 + 1 = 0
$$
$$
x^3 + 8 = 0
$$

Risolvo la prima: devo [scomporre](../../a/ad/ad6db.html)
$$
x^3 + 1 = (x + 1)(x^2 - x + 1) = 0
$$
Per la legge di [annullamento del prodotto](../../b/ba/bae2.html) per trovare tutte le soluzioni dovrò porre
- $x + 1 = 0$
- $x^2 - x + 1 = 0$

Risolvo la prima equazione:
$x = -1$

Risolvo la seconda equazione:
$x^2 - x + 1 = 0$
$$
x_{1,2} = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}
$$
$$
x_{1,2} = \frac{1 \pm \sqrt{1 - 4}}{2}
$$
$$
x_{1,2} = \frac{1 \pm \sqrt{-3}}{2}
$$
$$
x_{1,2} = \frac{1 \pm i\sqrt{3}}{2}
$$

Ho quindi le tre soluzioni
$x_1 = -1 \quad x_2 = (1 - i\sqrt{3})/2 \quad x_3 = (1 + i\sqrt{3})/2$

***

Risolvo la seconda: devo [scomporre](../../a/ad/ad6db.html)
$$
x^3 + 8 = (x + 2)(x^2 - 2x + 4) = 0
$$
Per la legge di [annullamento del prodotto](../../b/ba/bae2.html) per trovare tutte le soluzioni dovrò porre
- $x + 2 = 0$
- $x^2 - 2x + 4 = 0$

Risolvo la prima equazione:
$x = -2$

Risolvo la seconda equazione
$x^2 - 2x + 4 = 0$
applico la formula ridotta
$$
x_{1,2} = \frac{1 \pm \sqrt{1 - 4}}{1}
$$
$$
x_{1,2} = 1 \pm \sqrt{-3}
$$
$$
x_{1,2} = 1 \pm i\sqrt{3}
$$

Ho quindi le tre soluzioni
$x_1 = -2 \quad x_2 = 1 - i\sqrt{3} \quad x_3 = 1 + i\sqrt{3}$

***

In accordo con il [teorema fondamentale dell'algebra](afda.html) ho 6 soluzioni
$x_1 = -1 \quad x_2 = (1 - i\sqrt{3})/2 \quad x_3 = (1 + i\sqrt{3})/2$
$x_4 = -2 \quad x_5 = 1 - i\sqrt{3} \quad x_6 = 1 + i\sqrt{3}$

Due soluzioni sono reali e 4 complesse