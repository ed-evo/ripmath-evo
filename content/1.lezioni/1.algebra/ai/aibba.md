## Sistema di $$3$$ equazioni a tre incognite: metodo di sostituzione

Si tratta di adattare il metodo di sostituzione già visto per il sistema di due equazioni a due incognite.
Sceglieremo un'equazione per ricavare una incognita e sostituiremo il suo valore nelle altre due equazioni; in questo modo avremo due equazioni in due incognite e procederemo come già visto.

---

Vediamo ora su un esempio come risolvere un sistema di tre equazioni in tre incognite:

$$
\begin{cases}
\textcolor{blue}{x + y + z = 6} \\
\textcolor{blue}{2x + y - z = 1} \\
\textcolor{blue}{2x - 3y + z = -1}
\end{cases}
$$

Ricavo la $$z$$ dalla prima equazione e sostituisco nella seconda e nella terza (posso ricavare chi mi pare):

$$
\begin{cases}
\textcolor{red}{z = 6 - x - y} \\
\textcolor{red}{2x + y - (6 - x - y) = 1} \\
\textcolor{red}{2x - 3y + 6 - x - y = -1}
\end{cases}
$$

Al posto della prima equazione metto una linea (così non la uso più sino alla fine):

$$
\begin{cases}
\textcolor{red}{\text{----------------------}} \\
\textcolor{red}{2x + y - 6 + x + y = 1} \\
\textcolor{red}{2x - 3y + 6 - x - y = -1}
\end{cases}
$$

Porto i numeri dopo l'uguale e poi sommo:

$$
\begin{cases}
\textcolor{red}{\text{--------------}} \\
\textcolor{red}{3x + 2y = 7} \\
\textcolor{red}{x - 4y = -7}
\end{cases}
$$

Da notare che ho un sistema di due equazioni in due incognite.
Ora ricavo la $$x$$ dalla terza equazione:

$$
\begin{cases}
\textcolor{red}{\text{--------------}} \\
\textcolor{red}{3x + 2y = 7} \\
\textcolor{red}{x = 4y - 7}
\end{cases}
$$

Sostituisco il valore della $$x$$ trovato nella seconda equazione:

$$
\begin{cases